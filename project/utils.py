...
def type_specific_args(item: DataMountModel):
    ...
    if type_ == "webdav" and vendor_ == "nextcloud" and ( url_.endswith("/webdav") or url_.endswith("/webdav/")):
...
def get_cmd(item: DataMountModel, config_path: str):
    ...
    cmd_args = ["--vfs-cache-max-size=10G", "--vfs-read-chunk-size=64M", "--vfs-cache-mode=writes", "--allow-other", f"--uid={uid}", f"--gid={gid}"]    
    cmd = ["rclone", "mount", "--config", config_path, f"{template}:{remotepath}", fullpath] + cmd_args
...
async def create_config(item: DataMountModel):
    skip_keys = {"readonly", "displayName", "remotepath"} # They're used in the command as arguments, not in the config file itself
    config = {k: v for k, v in deepcopy(item.options.get("config", {})).items() if k not in skip_keys}
...
async def check_rclone_config(item: DataMountModel, config_path: str):
    ...
    template = item.options.get("template", None)
    remotepath = item.options.get("config", {}).get("remotepath", "None")
    cmd = ["rclone", "lsd", "--config", config_path, f"{template}:{remotepath}"] + type_specific_args(item)
